...
function SkillSetsComponent(props){
    useEffect(() => {
        props.fetchSkillSets();
    },[])

    const renderTechSkills = (items) => {
        return (
            items.map(data =>
    ...
    const renderTechStacks = (results) => {
        if (results.length > 0) {
            return (
                results.map(item =>
        ...
        var results = JSON.parse(JSON.stringify(props.skills)).data;
        return (
